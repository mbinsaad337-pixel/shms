...
<head>
    ...
    <style>
        ...
        .btn-navy { background:linear-gradient(135deg,#004274,#0a3d62); color:white; padding:10px 24px; border-radius:12px; font-weight:700; transition:all 0.3s; display:inline-flex; align-items:center; gap:8px; }
...
<header :class="scrolled ? 'glass shadow-md py-3' : 'bg-transparent py-5'" class="fixed w-full z-50 transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <div class="flex items-center gap-3">
            @auth
                {{-- <a href="{{ url('/dashboard') }}" class="btn-navy hidden sm:inline-flex text-sm"><i class="fas fa-desktop"></i> لوحة التحكم</a> --}}
            @else
                <a href="{{ route('login') }}" class="btn-navy hidden sm:inline-flex text-sm"><i class="fas fa-lock"></i> تسجيل الدخول</a>
            @endauth
            {{-- <button @click="mobileOpen=!mobileOpen" class="md:hidden text-gray-700 text-2xl"><i class="fas fa-bars"></i></button> --}}
        </div>
    </div>
    <!-- Mobile Menu -->
    <div x-show="mobileOpen" x-collapse class="md:hidden glass border-t border-gray-200">
        <div class="flex flex-col items-center gap-1 py-4 font-bold text-gray-700 text-sm">
            ...
            @auth
                <a href="{{ url('/dashboard') }}" class="btn-navy w-3/4 justify-center"><i class="fas fa-desktop"></i> لوحة التحكم</a>
            @else
                <a href="{{ route('login') }}" class="btn-navy w-3/4 justify-center"><i class="fas fa-lock"></i> تسجيل الدخول</a>
            @endauth
        </div>
    </div>
</header>
...
<!-- CENTERS SECTION -->
<section id="centers" class="py-24 bg-[#f8fafc]" x-data="{sel:null,open(c){this.sel=c;document.body.style.overflow='hidden'},close(){this.sel=null;document.body.style.overflow=''}}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            ...
            <p class="text-gray-500 font-almarai max-w-xl mx-auto">بيئة سكنية محفزة تدعم الطالب وتوفر له كافة سبل الراحة والتميز الأكاديمي</p>
        </div>
