...
<head>
    ...
    <style>
        ...
        .btn-navy { background:linear-gradient(135deg,#004274,#0a3d62); color:white; padding:10px 24px; border-radius:12px; font-weight:700; transition:all 0.3s; align-items:center; gap:8px; }
        .btn-navy:hover { transform:translateY(-2px); box-shadow:0 10px 20px rgba(0,66,116,0.25); color:#D4A044; }
        .text-navy { color:#004274; }
        .text-gold { color:#D4A044; }
        .bg-navy { background:#004274; }
        .bg-gold { background:#D4A044; }
        .border-gold { border-color:#D4A044; }
        .hover-lift { transition:all 0.35s cubic-bezier(.4,0,.2,1); }
        .hover-lift:hover { transform:translateY(-8px); box-shadow:0 20px 30px -8px rgba(0,66,116,0.15); }
        .hide-scrollbar::-webkit-scrollbar { display:none; }
        .hide-scrollbar { -ms-overflow-style:none; scrollbar-width:none; }
        /* News cards */
        .news-card { background:white; border-radius:1.5rem; overflow:hidden; box-shadow:0 2px 12px rgba(0,66,116,0.06); border:1px solid #eef2f7; transition:transform 0.3s,box-shadow 0.3s; display:flex; flex-direction:column; height:100%; }
        .news-card:hover { transform:translateY(-6px); box-shadow:0 16px 40px rgba(0,66,116,0.14); }
        .news-cover { width:100%; height:210px; overflow:hidden; background:#e9f0f6; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
        ...
        .read-more-btn { display:inline-flex; align-items:center; gap:6px; margin-top:14px; padding:8px 18px; background:#004274; color:white; border-radius:999px; font-size:0.8rem; font-weight:800; font-family:'Cairo',sans-serif; text-decoration:none; transition:background 0.2s,transform 0.2s; }
        .read-more-btn:hover { background:#D4A044; color:#004274; transform:translateY(-2px); }
        .center-details-button { background:rgba(0,66,116,0.05); color:#004274; }
        .center-details-button:hover { background:#004274; color:#D4A044 !important; }
        ...
        .badge-general { background:#e0edf7; color:#004274; }
        .badge-jobs { background:#d1fae5; color:#065f46; }
        /* Ticker */
        @keyframes ticker { 0%{transform:translateX(0)} 100%{transform:translateX(-50%)} }
        .ticker-scroll { animation:ticker 30s linear infinite; display:inline-flex; align-items:center; }
...
<!-- HEADER -->
<header :class="scrolled ? 'glass shadow-md py-3' : 'bg-transparent py-5'" class="fixed w-full z-50 transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        <div class="flex items-center gap-3">
            ...
            <div class="hidden sm:block font-black text-navy text-sm leading-tight "> جمعية رعاية طالب العلم<br><span class="text-xs font-bold text-gold-500" style="color:goldenrod;font-size:17.3px">Student Care Society</span></div>
        </div>
        ...
        <div class="flex items-center gap-3">
            @auth
                <a href="{{ url('/dashboard') }}" class="btn-navy hidden md:inline-flex text-sm"><i class="fas fa-desktop"></i> لوحة التحكم</a>
            @else
                <a href="{{ route('login') }}" class="btn-navy hidden md:inline-flex text-sm"><i class="fas fa-lock"></i> تسجيل الدخول</a>
            @endauth
            <button @click="mobileOpen=!mobileOpen" class="md:hidden text-gray-700 text-2xl px-2"><i class="fas fa-bars"></i></button>
        </div>
    </div>
    <!-- Mobile Menu -->
    <div x-show="mobileOpen" x-collapse class="md:hidden glass border-t border-gray-200">
        <div class="flex flex-col items-center gap-1 py-4 font-bold text-gray-700 text-sm">
            <a href="#hero" @click="mobileOpen=false" class="w-full text-center py-3 border-b border-gray-100 hover:text-navy">الرئيسية</a>
            <a href="#centers" @click="mobileOpen=false" class="w-full text-center py-3 border-b border-gray-100 hover:text-navy">المراكز</a>
            <a href="#about" @click="mobileOpen=false" class="w-full text-center py-3 border-b border-gray-100 hover:text-navy">من نحن</a>
            <a href="#news" @click="mobileOpen=false" class="w-full text-center py-3 border-b border-gray-100 hover:text-navy">الأخبار</a>
            <a href="#contact" @click="mobileOpen=false" class="w-full text-center py-3 mb-2 hover:text-navy">تواصل معنا</a>
            @auth
                <a href="{{ url('/dashboard') }}" class="btn-navy inline-flex w-3/4 justify-center"><i class="fas fa-desktop"></i> لوحة التحكم</a>
            @else
                <a href="{{ route('login') }}" class="btn-navy inline-flex w-3/4 justify-center"><i class="fas fa-lock"></i> تسجيل الدخول</a>
            @endauth
        </div>
    </div>
</header>
...
<section id="heroSection" class="relative overflow-hidden bg-gray-900" style="height:28vh">
    @foreach($slides as $i => $s)
    <div class="slider-slide absolute inset-0 transition-opacity duration-1000"
         style="{{ $i === 0 ? 'opacity:1;z-index:1' : 'opacity:0;z-index:0' }}">
        <img src="{{ $s['img'] }}" class="absolute inset-0 w-full h-full object-cover object-center"
             alt="شريحة {{ $i+1 }}" loading="{{ $i === 0 ? 'eager' : 'lazy' }}">
        <div class="absolute inset-0 bg-black/20 pointer-events-none"></div>
    </div>
    @endforeach

    <button onclick="sliderPrev()" class="absolute top-1/2 -translate-y-1/2 right-2 md:right-8 z-30 w-10 h-10 md:w-12 md:h-12 rounded-full bg-black/30 hover:bg-black/50 text-white backdrop-blur-sm flex items-center justify-center transition">
        <i class="fas fa-chevron-right text-sm"></i>
    </button>
    <button onclick="sliderNext()" class="absolute top-1/2 -translate-y-1/2 left-2 md:left-8 z-30 w-10 h-10 md:w-12 md:h-12 rounded-full bg-black/30 hover:bg-black/50 text-white backdrop-blur-sm flex items-center justify-center transition">
        <i class="fas fa-chevron-left text-sm"></i>
    </button>

    <div class="absolute bottom-4 md:bottom-8 z-30 flex gap-2 justify-center w-full px-4">
        @foreach($slides as $i => $s)
        <button onclick="sliderGo({{ $i }})" data-dot="{{ $i }}"
            class="slider-dot h-2 md:h-3 rounded-full transition-all duration-300"
            style="{{ $i === 0 ? 'width:1.5rem;background:#D4A044' : 'width:0.5rem;background:rgba(255,255,255,0.5)' }}">
        </button>
        @endforeach
    </div>
</section>
...
<style>
    @media (min-width:640px)  { #heroSection { height:40vh  !important; } }
    @media (min-width:768px)  { #heroSection { height:65vh  !important; } }
    @media (min-width:1024px) { #heroSection { height:100vh !important; } }
</style>
...
(function(){
    ...
    function go(n){
        ...
        document.querySelectorAll('.slider-slide').forEach(function(el, i){
            ...
            el.style.zIndex  = (i===cur)?'1':'0';
        });
        document.querySelectorAll('[data-dot]').forEach(function(el){
            var idx = parseInt(el.getAttribute('data-dot'));
            if(idx===cur){ el.style.width='1.5rem'; el.style.background='#D4A044'; }
            else          { el.style.width='0.5rem'; el.style.background='rgba(255,255,255,0.5)'; }
        });
    }

    window.sliderNext = function(){ go(cur+1); resetTimer(); };
    window.sliderPrev = function(){ go(cur-1); resetTimer(); };
    window.sliderGo   = function(n){ go(n); resetTimer(); };

    function resetTimer(){ clearInterval(timer); timer=setInterval(window.sliderNext,5000); }

    var tx=0, el=document.getElementById('heroSection');
    if(el){
        el.addEventListener('touchstart',function(e){ tx=e.changedTouches[0].screenX; },{passive:true});
        el.addEventListener('touchend',function(e){
            var d=e.changedTouches[0].screenX-tx;
            if(d>50) window.sliderPrev(); else if(d<-50) window.sliderNext();
        },{passive:true});
    }
    resetTimer();
})();
...
<section id="centers" class="py-12 md:py-24 bg-[#f8fafc]" x-data="{
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-8 md:mb-16">
            ...
            <div class="w-16 md:w-20 h-1.5 bg-gold rounded-full mx-auto mb-4"></div>
            <p class="text-gray-500 font-almarai max-w-xl mx-auto text-sm md:text-base">بيئة سكنية محفزة تدعم الطالب وتوفر له كافة سبل الراحة والتميز الأكاديمي</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($centers as $center)
            <div class="hover-lift bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm flex flex-col h-full">
                <div class="h-44 bg-gradient-to-br from-[#004274]/5 to-[#D4A044]/10 flex items-center justify-center p-6 relative">
                    @if($center->logo)
                        <img src="{{ asset('storage/'.$center->logo) }}" class="w-full h-full max-w-[80%] max-h-28 object-contain drop-shadow-md" alt="{{ $center->name }}">
                    @else
                        <i class="fas fa-building text-7xl text-navy/20"></i>
                    @endif
                    <span class="absolute top-4 left-4 text-xs font-bold px-3 py-1 rounded-full {{ $center->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                        <i class="fas {{ $center->is_active ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>{{ $center->is_active ? 'متاح' : 'مغلق' }}
                    </span>
                </div>
                <div class="p-6 flex flex-col flex-1">
                    <h3 class="text-xl font-black text-navy mb-2">{{ $center->name }}</h3>
                    <p class="text-gray-400 font-almarai text-sm mb-4 flex items-center gap-1"><i class="fas fa-map-marker-alt text-gold"></i> {{ $center->address }}</p>
            ...
            @empty
            <div class="col-span-3 text-center py-16 text-gray-400 font-almarai">لا توجد مراكز نشطة حالياً</div>
            @endforelse
        </div>
    </div>
    ...
    <div x-show="sel" style="display:none" class="fixed inset-0 z-[200] flex items-center justify-center px-4 sm:p-6">
        ...
        <div x-show="sel"
            ...
            <div class="p-3 sm:p-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center flex-wrap gap-2">
                ...
                <a :href="sel?.location_link ? sel.location_link : ('https://maps.google.com/?q='+(sel?sel.address:''))" target="_blank" class="btn-navy inline-flex px-4 py-2 sm:px-5 text-xs sm:text-sm rounded-xl">
...
<section id="about" class="py-12 md:py-24 bg-white relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-8 md:gap-16 items-stretch">
            <div class="flex flex-col justify-center">
                <span class="inline-block bg-navy/5 text-navy text-sm font-bold px-4 py-2 rounded-full mb-4 self-start">من نحن؟</span>
                <h2 class="text-2xl md:text-4xl font-black text-navy mb-4 leading-snug">جمعية رعاية طالب العلم</h2>
                <div class="w-16 h-1.5 bg-gold rounded-full mb-4 md:mb-6"></div>
                <p class="text-gray-600 font-almarai text-base md:text-lg leading-relaxed mb-6 md:mb-8">
                ...
                <div class="grid grid-cols-2 gap-3 md:gap-6 mb-6 md:mb-8 font-almarai">
                    <div class="p-3 md:p-4 rounded-2xl border border-navy/10 bg-navy/3 hover:border-gold transition">
                        <div class="w-9 h-9 md:w-10 md:h-10 bg-navy/10 text-navy rounded-lg flex items-center justify-center mb-2 md:mb-3"><i class="fas fa-eye text-sm md:text-base"></i></div>
                        <h4 class="font-bold text-gray-800 mb-1 text-sm md:text-base">رؤيتنا</h4>
                        <p class="text-xs text-gray-500">فرص تعليمية مستدامة لمخرجات وطنية فاعلة</p>
                    </div>
                    <div class="p-3 md:p-4 rounded-2xl border border-gold/20 bg-gold/5 hover:border-gold transition">
                        <div class="w-9 h-9 md:w-10 md:h-10 bg-gold/15 text-gold rounded-lg flex items-center justify-center mb-2 md:mb-3"><i class="fas fa-bullseye text-sm md:text-base"></i></div>
                        <h4 class="font-bold text-gray-800 mb-1 text-sm md:text-base">رسالتنا</h4>
                        <p class="text-xs text-gray-500">تمكين الأفراد من مواصلة تعليمهم وتطوير قدراتهم لبناء وطنهم.</p>
                    </div>
                    <div class="p-3 md:p-4 rounded-2xl border border-gray-100 bg-gray-50 hover:border-gold transition">
                        <div class="w-9 h-9 md:w-10 md:h-10 bg-emerald-100 text-emerald-600 rounded-lg flex items-center justify-center mb-2 md:mb-3"><i class="fas fa-list-check text-sm md:text-base"></i></div>
                        <h4 class="font-bold text-gray-800 mb-1 text-sm md:text-base">أهدافنا</h4>
                        <ul class="list-disc list-inside text-xs md:text-sm text-gray-500 space-y-1 mt-2">
                            ...
                            <li class="hidden md:list-item">تطوير الأداء المؤسسي.</li>
                            <li class="hidden md:list-item">الإبداع في إعداد وتنفيذ المشاريع التعليمية.</li>
                            <li class="hidden md:list-item">تعزيز الشراكات لدعم التعليم.</li>
                            <li class="hidden md:list-item">الاستدامة في المشاريع التعليمية.</li>
                        </ul>
                    </div>
                    <div class="p-3 md:p-4 rounded-2xl border border-gray-100 bg-gray-50 hover:border-gold transition">
                        <div class="w-9 h-9 md:w-10 md:h-10 bg-purple-100 text-purple-600 rounded-lg flex items-center justify-center mb-2 md:mb-3"><i class="fas fa-gem text-sm md:text-base"></i></div>
                        <h4 class="font-bold text-gray-800 mb-2 text-sm md:text-base">قيمنا</h4>
                        <div class="flex flex-wrap gap-1.5">
                            <span class="px-2 py-1 bg-purple-100/50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">الاستدامة</span>
                            <span class="px-2 py-1 bg-purple-100/50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">الشراكة</span>
                            <span class="px-2 py-1 bg-purple-100/50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">الفريق</span>
                            <span class="px-2 py-1 bg-purple-100/50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">الشفافية</span>
                            <span class="px-2 py-1 bg-purple-100/50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100">التمكين</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="relative px-4">
                <div class="relative w-full">
                    ...
                    <div class="absolute -bottom-3 -left-3 md:-bottom-4 md:-left-4 h-full w-full rounded-3xl border-4 border-gold/40 z-0"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 text-center">
            <div class="p-3 md:p-0">
                <div class="w-10 h-10 md:w-14 md:h-14 mx-auto bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-3 text-gold text-xl md:text-2xl"><i class="fas fa-building"></i></div>
                <div class="text-2xl md:text-4xl font-black text-gold mb-1">{{ $stats['centers'] }}</div>
                <p class="text-xs md:text-sm font-almarai text-gray-300">مراكز طلابية</p>
            </div>
            <div class="p-3 md:p-0">
                <div class="w-10 h-10 md:w-14 md:h-14 mx-auto bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-3 text-gold text-xl md:text-2xl"><i class="fas fa-user-graduate"></i></div>
                <div class="text-2xl md:text-4xl font-black text-gold mb-1">+{{ $center->students_count }}</div>
                <p class="text-xs md:text-sm font-almarai text-gray-300">طالب مستفيد</p>
            </div>
            <div class="p-3 md:p-0">
                <div class="w-10 h-10 md:w-14 md:h-14 mx-auto bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-3 text-gold text-xl md:text-2xl"><i class="fas fa-user-graduate"></i></div>
                <div class="text-2xl md:text-4xl font-black text-gold mb-1">+{{ $stats['graduates'] }}</div>
                <p class="text-xs md:text-sm font-almarai text-gray-300">طالب خريج</p>
            </div>
            <div class="p-3 md:p-0">
                <div class="w-10 h-10 md:w-14 md:h-14 mx-auto bg-white/10 rounded-xl md:rounded-2xl flex items-center justify-center mb-2 md:mb-3 text-gold text-xl md:text-2xl"><i class="fas fa-award"></i></div>
                <div class="text-2xl md:text-4xl font-black text-gold mb-1">+30</div>
                <p class="text-xs md:text-sm font-almarai text-gray-300">تخصص مختلف</p>
            </div>
        </div>
    </div>
</section>
...
<section id="news" class="py-12 md:py-24 bg-[#f8fafc]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <!-- Section Header -->
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-6 md:mb-10">
            <div>
                <div class="inline-flex items-center gap-2 bg-navy/5 text-navy px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-bold mb-2 md:mb-3">
                    <i class="fas fa-newspaper text-gold"></i> أخبار المراكز الطلابية
                </div>
                <h2 class="text-2xl md:text-4xl font-black text-navy">آخر الأخبار والفعاليات</h2>
                <div class="w-16 md:w-20 h-1.5 bg-gold rounded-full mt-2 md:mt-3"></div>
            </div>
            <div class="w-full md:w-auto">
                <select id="centerFilter" onchange="filterNews()" class="w-full md:w-auto border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-almarai focus:ring-2 focus:ring-[#004274] shadow-sm bg-white">
        ...
        <div id="tickerBar" class="hidden bg-navy rounded-2xl overflow-hidden mb-10 border border-navy/30">
            <div class="flex items-center gap-0">
                <div class="shrink-0 flex items-center gap-2 bg-gold text-navy px-5 py-2.5 font-black text-xs rounded-r-xl">
        ...
        <!-- Empty State -->
        <div id="newsEmpty" class="hidden text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
...
<footer class="bg-gray-900 text-white pt-16 pb-8 border-t-4 border-gold" id="contact">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
            ...
            <div>
                ...
                <ul class="space-y-3 font-almarai text-sm text-gray-400">
                    ...
                    <li><a href="#about" class="hover:text-white transition block"><i class="fas fa-angle-left mr-2 text-xs"></i>من نحن</a></li>
                    <li><a href="#news" class="hover:text-white transition block"><i class="fas fa-angle-left mr-2 text-xs"></i>الأخبار والفعاليات</a></li>
                    <li><a href="#contact" class="hover:text-white transition block"><i class="fas fa-angle-left mr-2 text-xs"></i>تواصل معنا</a></li>
                </ul>
            </div>
            <div>
        ...
        <div class="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center text-sm font-almarai text-gray-500">
...
<!-- Back to top -->
<button id="backToTop" onclick="window.scrollTo({top:0,behavior:'smooth'})" class="hidden fixed bottom-8 left-8 z-50 w-12 h-12 rounded-full bg-navy text-white shadow-xl hover:bg-gold hover:-translate-y-1 transition-all flex items-center justify-center text-xl">
...
<script>
    ...
    const categoryInfo = {
        sports:      { label:'رياضي',   cls:'badge-sports',      emoji:'⚽' },
        culture:     { label:'ثقافي',   cls:'badge-culture',     emoji:'📚' },
        achievement: { label:'إنجاز',   cls:'badge-achievement', emoji:'🏆' },
        jobs:        { label:'فرص عمل',cls:'badge-jobs',         emoji:'💼' },
        general:     { label:'عام',     cls:'badge-general',     emoji:'📰' },
    };

    function getCatInfo(cat) { return categoryInfo[cat] || categoryInfo.general; }
    ...
    function filterNews() { currentPage = 1; fetchNews(); }
    ...
    function fetchNews() {
        ...
        fetch(url)
            ...
            .catch(e => { console.error(e); loader.classList.add('hidden'); empty.classList.remove('hidden'); });
    }

    function buildCard(item) {
        const info = getCatInfo(item.category);
        const date = item.published_at
            ? new Date(item.published_at).toLocaleDateString('ar-SA',{year:'numeric',month:'short',day:'numeric'})
            : '';
    ...
    function buildPagination(cur, last) {
        ...
        for (let i = 1; i <= last; i++) {
            html += `<button onclick="currentPage=${i};fetchNews()" class="w-9 h-9 rounded-full font-bold text-sm transition ${i===cur?'bg-navy text-white':'bg-white text-gray-500 hover:bg-gray-100'}">${i}</button>`;
        }
        el.innerHTML = html;
    }
