    ...
    <tr>
        <td style="border: none; padding: 6px;">
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 8px 12px; text-align: center;">
                <span style="font-size: 10px; color: #64748b; font-weight: bold;">إجمالي المبالغ المالية للسجلات المعروضة: </span>
                <span style="font-size: 13px; font-weight: bold; color: #15803d;">{{ $stats['إجمالي المبالغ المالية'] }}</span>
            </div>
        </td>
    </tr>
</table>
...
<table class="signatures-table avoid-break">
    <tr>
        <td>
            ...
            <div class="sign-name">{{ auth()->user()->name ?? 'النظام' }}</div>
        </td>
        <td>
            <div class="sign-line"></div>
            ...
            <div class="sign-name">
                @php
                    $centerId = $center?->id ?? auth()->user()->center_id;
                    $center_manager = $centerId ? \App\Models\User::role('center-manager')->where('center_id', $centerId)->first() : null;
                @endphp
                {{ $center_manager?->name ?? '' }}
            </div>
        </td>
