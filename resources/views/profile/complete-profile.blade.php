...
@push('styles')
    <style>
        ...
        .step-dot {
            @apply w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all;
        }
        ...
        .progress-bar-bg {
            @apply w-full bg-gray-200 rounded-full h-2 mb-4 dark:bg-gray-700;
        }

        .progress-bar-fill {
            @apply bg-primary h-2 rounded-full transition-all duration-500 ease-in-out;
        }

        .step-indicator {
            @apply flex justify-between text-xs font-bold font-cairo text-gray-400 mb-6;
        }
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 py-8" dir="rtl">
        <div class="container mx-auto px-4 max-w-4xl">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-gray-500 font-almarai text-sm mt-1">يرجى تعبئة جميع البيانات بدقة — ستُراجع من قِبل الإدارة
                </p>
            </div>
            ...
            <form action="{{ route('profile.complete.update') }}" method="POST" enctype="multipart/form-data"
                class="space-y-6 wizard-form" id="profileWizardForm">
                <input type="hidden" name="profile_step" id="profile_step" value="{{ $student->profile_step ?? 1 }}">
                <input type="hidden" name="profile_completion" id="profile_completion"
                    value="{{ $student->profile_completion ?? 0 }}">
                @csrf
                    ...
                    <div class="bg-red-50 border-r-4 border-red-500 rounded-2xl p-6 mb-6 shadow-sm">
                        ...
                        <ul class="list-disc list-inside space-y-1">
                            @foreach ($errors->all() as $error)
                                <li class="text-red-600 font-almarai text-xs font-bold leading-relaxed">{{ $error }}
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="wizard-step active" data-step="1">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-blue-50 to-indigo-50">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الاسم الرباعي <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="name_ar" value="{{ old('name_ar', $student->name_ar) }}"
                                    required class="field-input @error('name_ar') border-red-500 @enderror">
                                @error('name_ar')
                                    <p class="text-red-500 text-[10px] font-bold mt-1 font-cairo"><i
                                            class="fas fa-exclamation-circle ml-1"></i> {{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">Full Name
                                    (English)</label>
                                <input type="text" name="name_en" value="{{ old('name_en', $student->name_en) }}"
                                    dir="ltr" class="field-input @error('name_en') border-red-500 @enderror"
                                    placeholder="As in Passport (Optional)">
                                @error('name_en')
                                    <p class="text-red-500 text-[10px] font-bold mt-1 font-cairo"><i
                                            class="fas fa-exclamation-circle ml-1"></i> {{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">تاريخ الميلاد <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">المدينة
                                    الحالية</label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">رقم البطاقة /
                                    الجواز</label>
                            ...
                            <div>
                                ...
                                <input type="text" name="id_card_source"
                                    value="{{ old('id_card_source', $student->id_card_source) }}"
                                ...
                                @error('id_card_source')
                                    <p class="text-red-500 text-[10px] font-bold mt-1 font-cairo"><i
                                            class="fas fa-exclamation-circle ml-1"></i> {{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">تاريخ
                                    البطاقة</label>
                                <input type="date" name="id_card_date"
                                    value="{{ old('id_card_date', $student->id_card_date?->format('Y-m-d')) }}"
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الجنسية <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الحالة
                                    الاجتماعية</label>
                                <select name="marital_status"
                                    ...
                                    @foreach (['single' => 'أعزب', 'married' => 'متزوج', 'divorced' => 'مطلق', 'widowed' => 'أرمل'] as $val => $lbl)
                                        <option value="{{ $val }}"
                                            {{ old('marital_status', $student->marital_status) == $val ? 'selected' : '' }}>
                                            {{ $lbl }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">نوع الدم</label>
            ...
            'O-' => 'O-',
                                        ...
                                        <option value="{{ $val }}"
                                            {{ old('blood_type', $student->blood_type) == $val ? 'selected' : '' }}>
                                            {{ $lbl }}</option>
                                    @endforeach
                                </select>
                                @error('blood_type')
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">عدد أفراد الأسرة
                                    المعالة</label>
                                <input type="number" name="dependents_count"
                                    value="{{ old('dependents_count', $student->dependents_count ?? 0) }}" min="0"
                                    max="30"
                                ...
                                <div class="flex gap-6">
                                    @foreach (['good' => 'جيدة', 'average' => 'متوسطة', 'weak' => 'ضعيفة'] as $val => $lbl)
                                        <label class="flex items-center gap-2 cursor-pointer group">
                                            ...
                                            <span
                                                class="font-almarai text-sm text-gray-700 group-hover:text-primary transition-colors">{{ $lbl }}</span>
                                        </label>
                                    @endforeach
                                </div>
                                @error('health_status')
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">رقم الجوال <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">البريد الإلكتروني
                    ...
                    <div class="flex justify-end gap-3 mt-6">
                        <button type="button"
                            class="btn-next bg-primary text-white px-8 py-3 rounded-xl font-bold font-cairo shadow-lg hover:bg-primary/90 transition-all flex items-center gap-2">التالي
                ...
                <div class="wizard-step" data-step="2">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-emerald-50 to-teal-50">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="governorate"
                                    value="{{ old('governorate', $student->governorate) }}"
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">العنوان
                                    التفصيلي</label>
                    ...
                    <div class="flex justify-between gap-3 mt-6">
                        <button type="button"
                            class="btn-prev bg-gray-200 text-gray-700 px-8 py-3 rounded-xl font-bold font-cairo hover:bg-gray-300 transition-all flex items-center gap-2"><i
                ...
                <div class="wizard-step" data-step="3">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-violet-50 to-purple-50">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">آخر شهادة حصل
                                    عليها</label>
                                <input type="text" name="last_certificate"
                                    value="{{ old('last_certificate', $student->last_certificate) }}"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all"
                                    placeholder="ثانوية عامة / بكالوريوس / ...">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">التخصص</label>
                                <input type="text" name="last_cert_major"
                                    value="{{ old('last_cert_major', $student->last_cert_major) }}"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all"
                                    placeholder="التخصص">
                            </div>
                            <div>
                                ...
                                <select name="last_cert_grade"
                                    ...
                                    @foreach (['امتياز', 'ممتاز', 'جيد جداً', 'جيد', 'مقبول', 'ضعيف'] as $g)
                                        <option value="{{ $g }}"
                                            {{ old('last_cert_grade', $student->last_cert_grade) == $g ? 'selected' : '' }}>
                                            {{ $g }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <input type="number" name="graduation_year"
                                    value="{{ old('graduation_year', $student->graduation_year) }}" min="1990"
                                    max="2030" placeholder="2024"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">المدرسة / الجهة
                                    المتخرج منها</label>
                                <input type="text" name="graduated_school"
                                    value="{{ old('graduated_school', $student->graduated_school) }}"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all"
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-amber-50 to-orange-50">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الرقم الجامعي <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الجامعة <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الكلية <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">التخصص <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">المستوى الدراسي
                                    <span class="text-red-500">*</span></label>
                                <select name="academic_level" required
                                    ...
                                    @foreach (['1' => 'الأول', '2' => 'الثاني', '3' => 'الثالث', '4' => 'الرابع', '5' => 'الخامس', '6' => 'السادس', '7' => 'السابع', '8' => 'الثامن', 'superior' => 'دراسات عليا'] as $v => $l)
                                        <option value="{{ $v }}"
                                            {{ old('academic_level', $student->academic_level) == $v ? 'selected' : '' }}>المستوى
                                            {{ $l }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">السنة الدراسية
                                    الحالية</label>
                                <input type="text" name="current_academic_year"
                                    value="{{ old('current_academic_year', $student->current_academic_year) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="study_duration"
                                    value="{{ old('study_duration', $student->study_duration) }}" placeholder="4 سنوات"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الفترة
                                    المتبقية</label>
                                <input type="text" name="remaining_period"
                                    value="{{ old('remaining_period', $student->remaining_period) }}" placeholder="سنتان"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">التاريخ المتوقع
                                    للتخرج</label>
                            ...
                            <div class="md:col-span-3">
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الأعمال والمهارات
                                    التي تجيدها</label>
                ...
                <div class="wizard-step" data-step="4">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-rose-50 to-pink-50">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">الاسم <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">صلة القرابة <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">رقم الهاتف <span
                                        class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">المستوى
                                    العلمي</label>
                            ...
                            <div class="md:col-span-3 pt-3 border-t border-gray-100 mt-2">
                                <p class="text-xs font-bold text-gray-500 font-cairo uppercase tracking-wide mb-3">جهة
                                    اتصال الطوارئ (اختياري - إذا كانت مختلفة)</p>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">اسم شخص
                                    الطوارئ</label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">رقم هاتف
                                    الطوارئ</label>
                ...
                <div class="wizard-step" data-step="5">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-cyan-50 to-sky-50">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">عدد
                                        الذكور</label>
                                    <input type="number" name="family_males"
                                        value="{{ old('family_males', $student->family_males ?? 0) }}" min="0"
                                        max="30"
                                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 font-cairo mb-1.5">عدد
                                        الإناث</label>
                                    <input type="number" name="family_females"
                                        value="{{ old('family_females', $student->family_females ?? 0) }}" min="0"
                                        max="30"
                                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary outline-none font-almarai text-sm bg-gray-50/50 transition-all">
                                </div>
                                <div>
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    <label class="text-sm font-bold text-gray-700 font-cairo">العاملون من أفراد
                                        الأسرة</label>
                                    ...
                                    <table class="w-full text-sm" id="workersTable">
                                        <thead class="bg-gray-50">
                                            <tr class="text-center">
                                                <th
                                                    class="px-3 py-2.5 font-cairo text-xs text-gray-500 font-bold border-b border-gray-200">
                                                    م</th>
                                                <th
                                                    class="px-3 py-2.5 font-cairo text-xs text-gray-500 font-bold border-b border-gray-200">
                                                    الاسم</th>
                                            ...
                                            @foreach ($existingWorkers as $i => $w)
                                                <tr class="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td class="px-2 py-2"><input type="text"
                                                            name="workers[{{ $i }}][name]"
                                                            value="{{ $w['name'] ?? '' }}" placeholder="الاسم"
                                                            class="w-full px-2 py-1.5 border border-gray-200 rounded-lg font-almarai text-xs focus:ring-1 focus:ring-primary outline-none bg-gray-50/50">
                                                    </td>
                                                    <td class="px-2 py-2"><input type="text"
                                                            name="workers[{{ $i }}][job]"
                                                            value="{{ $w['job'] ?? '' }}" placeholder="الوظيفة"
                                                            class="w-full px-2 py-1.5 border border-gray-200 rounded-lg font-almarai text-xs focus:ring-1 focus:ring-primary outline-none bg-gray-50/50">
                                                    </td>
                                                    <td class="px-2 py-2"><input type="text"
                                                            name="workers[{{ $i }}][organization]"
                                                            value="{{ $w['organization'] ?? '' }}" placeholder="المؤسسة"
                                                            class="w-full px-2 py-1.5 border border-gray-200 rounded-lg font-almarai text-xs focus:ring-1 focus:ring-primary outline-none bg-gray-50/50">
                                                    </td>
                                                    <td class="px-2 py-2"><input type="tel"
                                                            name="workers[{{ $i }}][phone]"
                                                            value="{{ $w['phone'] ?? '' }}" placeholder="الهاتف"
                                                            class="w-full px-2 py-1.5 border border-gray-200 rounded-lg font-almarai text-xs focus:ring-1 focus:ring-primary outline-none bg-gray-50/50">
                                                    </td>
                                                    <td class="px-2 py-2 text-center"><button type="button"
                                                            ...
                                                            class="text-red-400 hover:text-red-600 transition-colors"><i
                                                                class="fas fa-trash-alt text-xs"></i></button></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                </div>
                                <p class="text-xs text-gray-400 font-almarai mt-2">اترك الحقول فارغة إذا لم يكن هناك عاملون
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="wizard-step" data-step="6">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div
                            class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-l from-indigo-50 to-blue-50">
                            ...
                            <div>
                                ...
                                <p class="text-gray-400 text-xs font-almarai">يرجى رفع صور واضحة (JPG, PNG) أو ملفات PDF
                                </p>
                            </div>
                        </div>
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <div
                                        class="flex items-center gap-3 px-4 py-8 border-2 border-dashed border-gray-200 rounded-2xl group-hover:border-primary/50 transition-all bg-gray-50/30 group-hover:bg-primary/5">
                                        <div
                                            class="w-12 h-12 rounded-xl bg-white border border-gray-100 flex items-center justify-center text-primary shadow-sm">
                                            <i class="fas fa-user-circle text-xl"></i>
                                        </div>
                                        <div class="flex-1 overflow-hidden">
                                            <p class="text-xs font-bold text-gray-700 font-cairo truncate file-name">إختر
                                                صورة شخصية...</p>
                                            <p class="text-[10px] text-gray-400 font-almarai mt-0.5">JPG, PNG, PDF (بحد
                                                أقصى 10MB)</p>
                                        </div>
                                    </div>
                                </div>
                                @if ($student->photo)
                                    <p class="text-[10px] text-emerald-600 font-almarai px-2"><i
                                            class="fas fa-check-circle"></i> تم رفع ملف مسبقاً</p>
                                @endif
                            </div>

                            {{-- National ID --}}
                            <div class="space-y-2">
                                <label class="field-label">البطاقة الشخصية / الجواز <span
                                        class="text-red-500">*</span></label>
                                <div class="relative group">
                                    ...
                                    <div
                                        ...
                                        <div class="flex-1 overflow-hidden">
                                            <p class="text-xs font-bold text-gray-700 font-cairo truncate file-name">إختر
                                                ملف البطاقة...</p>
                                            <p class="text-[10px] text-gray-400 font-almarai mt-0.5">PDF or Image (Max
                                                10MB)</p>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="wizard-step" data-step="7">
                    <div class="bg-gradient-to-l from-gray-800 to-gray-900 rounded-2xl p-6 text-right">
                        ...
                        <p class="text-gray-300 text-xs font-almarai leading-relaxed mb-4">
                            أتعهد أنا الموقع أدناه بأن أكون على خلق حسن وتعامل طيب مع الآخرين، وأن أكون مجداً ومجتهداً في
                            دروسي وأن
                            أتلزم بلوائح وأنظمة المركز والتعامل الحسن مع الإدارة والمحافظة على نظافة المركز وأتحمل مسؤولية
                            أي محتويات
                        ...
                        <label class="flex items-start gap-3 cursor-pointer group">
                            ...
                            <span
                                class="text-gray-300 text-xs font-almarai leading-relaxed group-hover:text-white transition-colors">
                                أقر بصحة جميع المعلومات المدخلة وأتعهد بالالتزام باللوائح والأنظمة المعمول بها في المركز.
                                <span class="text-red-400">*</span>
                            </span>
                        </label>
                        @error('pledge')
                            <p class="text-red-400 text-[10px] font-bold mt-1 font-cairo"><i
                                    class="fas fa-exclamation-circle ml-1"></i> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="pb-8 flex flex-col md:flex-row gap-4 mt-6">
                        <button type="button"
                            class="btn-prev bg-gray-200 text-gray-700 px-8 py-4 rounded-2xl font-bold font-cairo hover:bg-gray-300 transition-all flex items-center justify-center gap-2 md:w-1/4"><i
                                class="fas fa-arrow-right"></i> السابق</button>
                    ...
                    <p class="text-center text-gray-400 text-xs font-almarai mt-3 mb-6">
                        ستُراجَع بياناتك من قِبل الإدارة قبل اعتمادها • الحقول المميزة بـ <span
                            class="text-red-500">*</span> مطلوبة
                    </p>
                </div>
    ...
    <script>
        ...
        function validateCurrentStep() {
            ...
            if (!isValid && firstInvalid) {
                ...
                showStatus('⚠️ يرجى تعبئة جميع الحقول المطلوبة بشكل صحيح', 'fa-exclamation-triangle text-red-500');
            }
        ...
        saveIndicator.className =
            "fixed bottom-4 left-4 bg-navy text-white px-4 py-2 rounded-xl text-xs font-bold font-cairo shadow-2xl z-[9999] transition-opacity opacity-0 flex items-center gap-2 border border-white/10";
        ...
        function showStatus(text, icon = 'fa-sync fa-spin text-white') {
            ...
            if (!icon.includes('spin')) {
                setTimeout(() => {
                    if (saveIndicator.style.opacity === '1' && !saveIndicator.innerHTML.includes('spin'))
        ...
        function saveDraftAjax(callback = null) {
            ...
            // Do not upload files on auto-save
        ...
        function restoreForm() {
            ...
            if (!savedDraft || currentStep > 1) return;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const errorElement = document.querySelector(
                '.bg-red-50.border-r-4, .text-red-500.font-bold, .border-red-500');
            if (errorElement) {
                const stepDiv = errorElement.closest('.wizard-step');
                if (stepDiv) {
                    const step = stepDiv.getAttribute('data-step');
                    if (step) {
