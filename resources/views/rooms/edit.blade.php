...
@section('title', 'تعديل بيانات المرفق')

@section('content')
    <div class="container mx-auto px-6 py-8">
        <div class="max-w-2xl mx-auto">
            <div class="mb-6">
                <a href="{{ route('rooms.index') }}" class="px-6 py-3 bg-gray-50 text-navy rounded-2xl hover:bg-gray-100 font-cairo font-bold transition-all inline-flex items-center gap-2 border border-gray-100">
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            <div class="bg-primary px-10 py-8 text-white relative">
                <h2 class="text-2xl font-bold font-cairo">تعديل بيانات المرفق</h2>
                <p class="text-white/80 font-almarai mt-2">تحديث بيانات الغرفة السكنية أو القاعات</p>
            </div>

            <form action="{{ route('rooms.update', $room) }}" method="POST" class="p-10 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="type" required
                            ...
                            <option value="residential" {{ $room->type == 'residential' ? 'selected' : '' }}>غرفة سكنية
                            </option>
                            <option value="study_hall" {{ $room->type == 'study_hall' ? 'selected' : '' }}>قاعة دراسية
                                (مذاكرة)</option>
                            <option value="activity_hall" {{ $room->type == 'activity_hall' ? 'selected' : '' }}>قاعة أنشطة /
                                تدريب</option>
                            <option value="other" {{ $room->type == 'other' ? 'selected' : '' }}>أخرى</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="apartment" value="{{ $room->apartment }}" placeholder="مثال: أ أو 1"
                            class="w-full px-5 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none font-almarai transition-all">
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-gray-700 font-cairo font-bold mb-2">حالة المرفق</label>
                    <select name="status" required
                        class="w-full px-5 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none font-almarai bg-white transition-all">
                        <option value="available" {{ $room->status == 'available' ? 'selected' : '' }}>متاح</option>
                        <option value="maintenance" {{ $room->status == 'maintenance' ? 'selected' : '' }}>في الصيانة</option>
                        <option value="closed" {{ $room->status == 'closed' ? 'selected' : '' }}>مغلق مؤقتاً</option>
                    </select>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-6">
                    ...
                    <button type="submit"
                        class="px-10 py-3 bg-secondary text-white rounded-xl hover:bg-orange-600 shadow-lg font-cairo font-bold transition-all transform hover:-translate-y-1 text-sm">تحديث
                        البيانات</button>
                </div>
            </form>
        </div>
