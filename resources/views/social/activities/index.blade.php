        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm mb-8 border border-gray-100">
            <form action="{{ route('activities.index') }}" method="GET"
                ...
                <div class="flex items-center gap-2">
                    <button type="submit"
                        class="flex-1 bg-navy text-white p-3 rounded-xl font-bold font-cairo hover:bg-navy/90 transition-all shadow-md">
                        <i class="fas fa-filter text-gold ml-1"></i> عرض
                    </button>
                    @if (request()->anyFilled(['center_id', 'month', 'category']))
                        <a href="{{ route('activities.index') }}"
                            class="w-12 h-11 flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-all border border-red-100"
                            title="إعادة تعيين">
                ...
                <table class="w-full text-right">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th
                                class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo whitespace-nowrap">
                                اسم النشاط</th>
                            <th
                                class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo whitespace-nowrap">
                                الجهة / المكان</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @if (is_countable($activities)
                                ? count($activities) > 0
                                : (method_exists($activities, 'count')
                                    ? $activities->count() > 0
                                    : !empty($activities)))
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            @if ($activity->club?->logo)
                                                <img src="{{ asset('storage/' . $activity->club->logo) }}"
                                                    class="w-10 h-10 rounded-xl object-contain bg-white shadow-sm border border-gray-50"
                                                    alt="logo">
                                            @else
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-navy/5 flex items-center justify-center text-navy/40">
                                                    <i class="fas fa-users-viewfinder"></i>
                                                </div>
                                            @endif
                                            <div>
                                                <a href="{{ route('activities.show', $activity->id) }}"
                                                    class="text-sm font-black text-navy font-cairo hover:text-gold transition-colors block">{{ $activity->name }}</a>
                                                @if ($activity->category)
                                                    <span
                                                        class="inline-block mt-1 px-2 py-0.5 bg-gold/10 text-gold rounded text-[10px] font-bold font-cairo">{{ $activity->category }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-xs font-bold text-gray-500 font-almarai mb-1"><i
                                                class="fas fa-building text-gold/50 ml-1"></i>{{ $activity->club->name ?? 'نادي عام' }}
                                        </div>
                                        <div class="text-xs text-gray-400 font-almarai"><i
                                                class="fas fa-location-dot text-navy/50 ml-1"></i>{{ $activity->location }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-xs font-bold text-gray-500 font-almarai mb-1"><i
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="inline-flex flex-col items-center">
                                            <span
                                                class="text-sm font-black text-navy">{{ $activity->participants->count() }}
                                                <span class="text-xs text-gray-400 font-normal">/
                                                    {{ $activity->targetedStudents->count() > 0 ? $activity->targetedStudents->count() : $activity->max_participants ?? '∞' }}</span></span>
                                            @if ($activity->target_audience)
                                                <span
                                                    class="text-[10px] text-gray-400 font-almarai mt-1">{{ $activity->target_audience }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if (!auth()->user()->hasRole('super-admin') && !auth()->user()->hasRole('activity-assistant'))
                                            ...
                                            @else bg-gray-50 text-gray-500 @endif">
                                                ...
                                                <i
                                                    class="fas fa-chevron-down absolute left-2 top-1/2 -translate-y-1/2 text-[8px] pointer-events-none opacity-50"></i>
                                            </form>
                                        @else
                                            <span
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            @if (auth()->user()->can('register-activities'))
                                                <button
                                                    onclick="openRegisterModal({{ $activity->id }}, '{{ addslashes($activity->name) }}')"
                                                    class="w-8 h-8 rounded-lg bg-gray-50 text-navy hover:bg-navy hover:text-white transition-all flex items-center justify-center"
                                                    title="تسجيل حضور">
                                            ...
                                            <a href="{{ route('activities.show', $activity->id) }}"
                                                class="w-8 h-8 rounded-lg bg-navy/5 text-navy hover:bg-navy hover:text-white transition-all flex items-center justify-center"
                                                title="عرض التفاصيل">
                                            ...
                                            @if (!auth()->user()->hasRole('super-admin') && !auth()->user()->hasRole('activity-assistant'))
                                                <a href="{{ route('activities.edit', $activity->id) }}"
                                                    class="w-8 h-8 rounded-lg bg-gold/10 text-gold hover:bg-gold hover:text-white transition-all flex items-center justify-center"
                        ...
                        @else
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-gray-400 font-almarai text-sm mt-2">ابدأ بجدولة أولى فعالياتك الاجتماعية
                                        للأندية والمراكز</p>
                                </td>
                            </tr>
                        @endif
                ...
                <div class="flex items-center justify-between mb-8 border-b border-gray-50 pb-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p class="text-gray-400 font-almarai text-sm ">يرجى تعبئة بيانات الفعالية والطلاب المستهدفين
                            </p>
                        </div>
                    </div>
                ...
                <form action="{{ route('activities.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            <label class="block text-sm font-black text-navy mb-3 font-cairo text-right">عنوان
                                الفعالية</label>
                        ...
                        <div>
                            <label class="block text-sm font-black text-navy mb-3 font-cairo text-right">النادي
                                المنظم</label>
                        ...
                        <div>
                            <label class="block text-sm font-black text-navy mb-3 font-cairo text-right">الموقع /
                                القاعة</label>
                        ...
                        <div class="bg-gray-50 rounded-[2rem] p-6 md:p-8 border border-gray-100 shadow-inner">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <div class="flex gap-3">
                                    <div id="club_member_option"
                                        class="hidden items-center gap-3 bg-white px-4 py-2 rounded-xl border border-gold/20 shadow-sm">
                                        <input type="checkbox" name="target_club_members" value="1"
                                            id="target_club"
                                            class="w-5 h-5 rounded border-gray-300 text-gold focus:ring-gold">
                                        <label for="target_club" class="text-xs font-bold text-navy font-cairo">أعضاء
                                            النادي</label>
                                    </div>
                                    <div
                                        class="flex items-center gap-3 bg-navy/5 px-4 py-2 rounded-xl border border-navy/10 shadow-sm">
                                        <input type="checkbox" name="target_all_students" value="1" id="target_all"
                                            class="w-5 h-5 rounded border-gray-300 text-navy focus:ring-navy">
                                        <label for="target_all" class="text-xs font-bold text-navy font-cairo">جميع طلاب
                                            المركز</label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6">
                                <div class="relative">
                                    <input type="text" id="student_search" placeholder="ابحث عن الطالب بالاسم..."
                                ...
                                <div class="space-y-2 max-h-80 overflow-y-auto px-2 custom-scrollbar-v2"
                                    ...
                                    @foreach ($students as $student)
                                        <label
                                            class="student-item group relative flex items-center justify-between p-4 rounded-2xl border border-white bg-white/50 hover:bg-white hover:border-gold/30 hover:shadow-md cursor-pointer transition-all">
                                            <div class="flex items-center gap-4">
                                                <input type="checkbox" name="target_student_ids[]"
                                                    value="{{ $student->id }}"
                                                    class="w-6 h-6 rounded-lg border-gray-300 text-gold focus:ring-gold transition-transform group-hover:scale-110">
                                            ...
                                            <div
                                                class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div id="registerModal"
        ...
        <div
            ...
            <form id="registerForm" method="POST" class="space-y-6">
                @csrf
                <div id="reader" style="width: 100%; border-radius: 1.5rem; overflow: hidden; display: none;"
                    class="mb-6 border-4 border-gold/20 shadow-inner"></div>
                ...
                <div class="relative">
                    <label class="block text-sm font-black text-navy mb-3 font-cairo text-center">أدخل باركود الطالب أو
                        الرقم الجامعي</label>
