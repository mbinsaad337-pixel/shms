    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="mb-8 flex justify-between items-center bg-white p-8 rounded-3xl border-l-8 border-navy shadow-sm">
            <div>
                ...
                <p class="text-gray-400 font-almarai text-sm mt-2 text-right">نادي: {{ $activity->club->name ?? 'نادي عام' }}
                </p>
            </div>
                ...
                @if (!auth()->user()->hasRole('super-admin') && !auth()->user()->hasRole('activity-assistant'))
                    <form action="{{ route('activities.update-status', $activity->id) }}" method="POST" class="relative">
                        ...
                        <select name="status" onchange="this.form.submit()"
                            class="pl-10 pr-6 py-3 bg-white rounded-2xl font-cairo font-bold transition-all border border-gray-200 shadow-sm appearance-none cursor-pointer focus:ring-0
                        ...
                        @else text-gray-500 @endif">
                            ...
                            <option value="published" {{ $activity->status == 'published' ? 'selected' : '' }}>مستمرة
                            </option>
                            <option value="completed" {{ $activity->status == 'completed' ? 'selected' : '' }}>منتهية
                            </option>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-50">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <span class="block text-xs text-gray-400 font-bold uppercase font-cairo">الفترة
                                    الزمنية</span>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <span class="block text-xs text-gray-400 font-bold uppercase font-cairo">المشاركون
                                    الملحقون</span>
                                <span class="text-navy font-black text-lg">{{ $activity->participants->count() }} /
                                    {{ $activity->targetedStudents->count() ?: $activity->max_participants ?? '∞' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <span
                                    class="px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-black uppercase tracking-wider">
                            ...
                            <div>
                                <span class="block text-xs text-gray-400 font-bold uppercase font-cairo">الجمهور
                                    المستهدف</span>
                                <span
                                    class="px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-black uppercase tracking-wider">
                    ...
                    @if ($activity->total_cost)
                        <div class="flex items-center gap-4 mt-6 pt-6 border-t border-gray-50">
                            ...
                            <div>
                                <span class="block text-xs text-gray-400 font-bold uppercase font-cairo">إجمالي
                                    التكلفة</span>
                                <span
                    ...
                    @if ($activity->attachment_pdf)
                        <div class="flex items-center gap-4 mt-4">
                            ...
                            <div>
                                <span class="block text-xs text-gray-400 font-bold uppercase font-cairo">ملف التصور</span>
                                <a href="{{ asset('storage/' . $activity->attachment_pdf) }}" target="_blank"
                                    class="text-red-500 font-bold font-cairo text-sm hover:underline flex items-center gap-1 mt-1">
                    ...
                    @if (!auth()->user()->hasRole('super-admin'))
                        <div class="mt-10 pt-8 border-t border-gray-50 flex flex-col gap-3">
                            <form action="{{ route('activities.destroy', $activity->id) }}" method="POST"
                                data-confirm="حذف الفعالية سيؤدي لإزالة سجل الحضور أيضاً. هل أنت متأكد؟">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full py-4 bg-red-50 text-red-500 rounded-2xl font-bold font-cairo hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2">
                    ...
                    <div class="bg-navy rounded-[2rem] p-8 shadow-xl text-white">
                        ...
                        <div class="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                            @foreach ($activity->targetedStudents as $target)
                                <div
                                    class="flex items-center justify-between py-2 border-b border-white/10 last:border-0 text-sm">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-8 py-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                        ...
                        <span
                            class="px-4 py-2 bg-navy text-white rounded-xl text-xs font-black">{{ $activity->participants->count() }}
                            طالب</span>
                    </div>
                    ...
                    <div class="p-0 overflow-x-auto">
                        <table class="w-full text-right">
                            <thead>
                                <tr class="bg-gray-50">
                                    <th
                                        class="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">
                                        اسم الطالب</th>
                                    <th
                                        class="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">
                                        الرقم الجامعي</th>
                                    <th
                                        class="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo text-center">
                                        وقت التحضير</th>
                                </tr>
                            </thead>
                                        ...
                                        <tr class="hover:bg-gray-50/50 transition-colors">
                                            <td class="px-8 py-5">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <span
                                                        class="text-navy font-bold font-almarai">{{ $participant->student->name_ar }}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-8 py-5 text-center">
                                                <span
                                                    class="px-3 py-1.5 bg-green-50 text-green-600 rounded-lg text-xs font-black  ">
                                ...
                                @else
                                    <tr>
                                        <td colspan="3" class="px-8 py-20 text-center">
                                            ...
                                            <p class="text-gray-400 font-almarai text-xs mt-2 italic">ابدأ بمسح الباركود من
                                                القائمة الرئيسية لتسجيل الحضور</p>
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
                    ...
                    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                        <div class="px-8 py-6 bg-red-50 border-b border-red-100 flex justify-between items-center">
                            <div>
                                ...
                                <span class="text-xs text-red-400 font-bold opacity-70">{{ $absentStudents->count() }}
                        ...
                        <form action="{{ route('violations.store') }}" method="POST">
                            ...
                            <input type="hidden" name="description"
                                value="التغيب عن حضور الفعالية/النشاط: {{ $activity->name }} (تاريخ: {{ $activity->start_date?->format('Y-m-d') }})">
                            <input type="hidden" name="violation_date"
                                value="{{ $activity->start_date?->format('Y-m-d') ?? now()->format('Y-m-d') }}">
                            ...
                            @if ($absentStudents->count() > 0 && !auth()->user()->hasRole('super-admin'))
                                <div class="p-6 border-b border-gray-50 flex items-end gap-6 bg-white">
                                    <div class="flex-[2]">
                                        <label class="block text-sm font-black text-navy mb-3 font-cairo text-right">اختر
                                            مستوى المخالفة للطلاب المحددين</label>
                                        <div class="relative">
                                            <div
                                            ...
                                            <select name="severity" required
                                                ...
                                                <option value="moderate">مخالفة متوسطة (إنذار / حرمان من بعض الامتيازات)
                                                </option>
                                    ...
                                    <button type="submit"
                                        class="flex-1 px-8 py-4 bg-red-600 text-white rounded-xl font-black font-cairo hover:bg-red-700 shadow-lg shadow-red-600/30 transition-all flex items-center justify-center gap-3">
                                    ...
                                    <thead>
                                        <tr class="bg-gray-50">
                                            <th class="px-8 py-4 w-16 text-center border-l border-gray-100">
                                                <input type="checkbox" id="selectAllAbsents"
                                                    class="w-5 h-5 rounded border-gray-300 text-red-600 focus:ring-red-600 cursor-pointer">
                                            </th>
                                            <th
                                                class="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">
                                                اسم الطالب</th>
                                                ...
                                                <tr class="hover:bg-red-50/50 transition-colors">
                                                    <td class="px-8 py-5 text-center border-l border-gray-50">
                                                        <input type="checkbox" name="student_id[]"
                                                            value="{{ $student->id }}"
                                                            class="absent-checkbox w-5 h-5 rounded border-gray-300 text-red-600 focus:ring-red-600 cursor-pointer">
                                                    </td>
                                                    <td class="px-8 py-5">
                                                        <div class="flex items-center gap-4">
                                                            <div
                                                                class="w-10 h-10 bg-red-100 text-red-600 rounded-xl flex items-center justify-center font-bold text-sm shadow-inner mt-1">
                                                                {{ mb_substr($student->name_ar, 0, 1) }}
                                                            </div>
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-navy font-bold font-almarai">{{ $student->name_ar }}</span>
                                                                <span class="text-[10px] text-gray-400 font-bold mt-1">لم
                                                                    يحضر</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                        ...
                                        @else
                                            <tr>
                                                <td colspan="3" class="px-8 py-16 text-center">
                                                    <div
                                                        class="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-5 text-green-500 shadow-sm border border-green-100">
                                                        <i class="fas fa-check-double text-4xl"></i>
                                                    </div>
                                                    <h4 class="text-navy text-xl font-black font-cairo">جميع المستهدفين
                                                        حاضرون</h4>
                                                    <p class="text-gray-400 font-almarai mt-2 italic">لا توجد حالات غياب عن
                                                        هذه الفعالية</p>
                                                </td>
                                            </tr>
                                        @endif
