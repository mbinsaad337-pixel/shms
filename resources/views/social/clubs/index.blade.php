...
@section('content')
            ...
            <form action="{{ route('clubs.index') }}" method="GET" class="flex gap-4 items-end">
                ...
                <div class="w-32">
                    @if(request()->filled('center_id'))
                        <a href="{{ route('clubs.index') }}" class="w-full flex items-center justify-center bg-red-50 text-red-500 rounded-xl hover:bg-red-100 transition-all border border-red-100 p-3" title="إعادة تعيين">
                    ...
                    <div class="p-8 flex-grow">
                        ...
                        <div class="flex items-center justify-between mt-auto border-t border-gray-50 pt-6">
                            <div class="flex flex-col">
                                <span class="text-[10px] font-black text-gray-400 uppercase font-cairo tracking-widest">عدد
                                    الأعضاء</span>
                            ...
                            <div class="flex gap-2">
                                ...
                                @if(!auth()->user()->hasRole('super-admin'))
                                    <a href="{{ route('clubs.edit', $club->id) }}"
                                        class="w-10 h-10 bg-gold/5 text-gold rounded-xl flex items-center justify-center hover:bg-gold hover:text-white transition-all"
                                        title="تعديل">
                                        <i class="fas fa-edit text-xs"></i>
                                    </a>
                                    <form action="{{ route('clubs.destroy', $club->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا النادي؟')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
    ...
    <!-- Create Club Modal -->
    <div id="clubModal" class="fixed inset-0 bg-navy/60 backdrop-blur-sm hidden items-start justify-center z-[1000] p-4 overflow-y-auto">
        <div
            ...
            <form action="{{ route('clubs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-black text-navy mb-3 font-cairo text-right">شعار النادي (اختياري)</label>
    ...
    <script>
        function openClubModal() { showModal('clubModal'); }
        function closeClubModal() { hideModal('clubModal'); }
