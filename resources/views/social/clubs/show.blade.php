...
@section('content')
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-right">
                            <thead class="bg-gray-50/50">
                                <tr>
                                    <th
                                        class="px-10 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">
                                        الطالب</th>
                                    <th
                                        class="px-10 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo text-center">
                                        الصفة / الدور</th>
                                    <th
                                        class="px-10 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">
                                        تاريخ الانضمام</th>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @if(is_countable($club->members) ? count($club->members) > 0 : (method_exists($club->members, 'count') ? $club->members->count() > 0 : !empty($club->members)))
    @foreach($club->members as $member)
                                    <tr class="hover:bg-gray-50/50 transition-colors group">
                                        <td class="px-10 py-6">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="w-12 h-12 bg-navy/5 rounded-xl flex items-center justify-center text-navy font-black text-sm">
                                                    {{ mb_substr($member->student->name_ar, 0, 1) }}
                                                </div>
                                                <div>
                                                    <p
                                                        class="font-black text-navy font-cairo group-hover:text-gold transition-colors">
    ...
    <div id="memberModal" class="fixed inset-0 bg-navy/60 backdrop-blur-sm hidden items-center justify-center z-[100] p-4">
        <div
            class="bg-white rounded-[2.5rem] p-10 max-w-xl w-full shadow-2xl transform transition-all border-t-8 border-gold">
