...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-3 mb-8">
            <a href="{{ route('news.index') }}" class="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-gray-400 hover:text-navy transition-all">
        ...
        <form action="{{ route('news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3" id="categoryGroup">
                        @foreach(['general' => ['label' => 'عام', 'icon' => 'fa-newspaper', 'color' => 'navy'],
                                  'sports'  => ['label' => 'رياضي', 'icon' => 'fa-futbol', 'color' => 'blue'],
                                  'culture' => ['label' => 'ثقافي', 'icon' => 'fa-book-open', 'color' => 'purple'],
                                  'achievement' => ['label' => 'إنجاز', 'icon' => 'fa-trophy', 'color' => 'amber']] as $val => $cat)
                            <label class="category-btn cursor-pointer p-4 rounded-2xl border-2 border-gray-100 text-center hover:border-navy transition-all {{ old('category') === $val ? 'border-navy bg-navy/5' : '' }}">
                                <input type="radio" name="category" value="{{ $val }}" class="sr-only" {{ old('category', 'general') === $val ? 'checked' : '' }}>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="relative border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center hover:border-navy transition-all cursor-pointer h-48 flex flex-col items-center justify-center" onclick="document.getElementById('cover_image').click()">
                    ...
                    <div>
                        ...
                        <div class="relative border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center hover:border-navy transition-all cursor-pointer h-48 flex flex-col items-center justify-center overflow-hidden" onclick="document.getElementById('gallery').click()">
                            ...
                            <input type="file" name="gallery[]" id="gallery" accept="image/*" multiple class="sr-only" onchange="previewGallery(event)">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-50">
                    {{-- Video URL --}}
                    <div>
                        <label class="block text-sm font-black text-navy mb-3 font-cairo">رابط فيديو (YouTube/YouTube)</label>
                        <div class="relative">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                <h3 class="font-black text-navy font-cairo text-lg border-b border-gray-100 pb-4 mb-6 flex items-center gap-2">
                    <i class="fas fa-paper-plane text-gold"></i> خيارات النشر
                </h3>
                <label class="flex items-center gap-4 cursor-pointer p-4 bg-emerald-50 rounded-2xl border border-emerald-100 hover:bg-emerald-100 transition-all">
                    ...
                    <div>
                        <p class="font-bold font-cairo text-emerald-800">نشر فوري</p>
                        <p class="text-sm font-almarai text-emerald-600">ستظهر هذه المادة في شريط الأخبار أسفل صفحة تسجيل الدخول فور الحفظ</p>
                    </div>
                </label>
            </div>
            ...
            <div class="flex gap-4">
                <button type="submit"
                    ...
                    <i class="fas fa-check-circle text-gold"></i> حفظ ونشر الخبر
                </button>
