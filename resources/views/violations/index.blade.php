...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm mb-8 animate-fade-in">
            <form action="{{ route('violations.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    <label class="block text-xs font-bold text-gray-400 mb-2 font-cairo mr-2">نوع المخالفة</label>
                    <select name="type" class="w-full px-5 py-3 rounded-xl border border-gray-100 bg-gray-50 text-navy font-almarai text-sm focus:ring-2 focus:ring-navy/5 outline-none">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="flex-1 bg-navy text-white py-3 rounded-xl font-bold font-cairo hover:bg-navy-light transition-all flex items-center justify-center gap-2 shadow-lg shadow-navy/10">
                        <i class="fas fa-filter text-xs"></i>
                        <span>فلترة</span>
                    </button>
                    @if(request()->anyFilled(['type', 'date_from', 'date_to']))
                        <a href="{{ route('violations.index') }}" class="w-12 h-12 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm" title="إلغاء الفلترة">
        ...
        <!-- Header Section -->
        <div class="mb-8 flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-3xl border-l-8 border-red-600 shadow-sm gap-4">
            <div>
                ...
                <table class="w-full text-right">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            ...
                            <th class="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo">نوع المخالفة</th>
                            <th class="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo text-center">المستوى</th>
                            <th class="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo text-center">التاريخ</th>
                            <th class="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest font-cairo text-center">بواسطة</th>
                        ...
                        @if(count($violations) > 0)
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex flex-col">
                                            <span class="text-navy font-bold font-almarai">{{ $violation->student->name_ar }}</span>
                                            <span class="text-[10px] text-gray-400 font-mono tracking-widest">{{ $violation->student->student_number }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <span class="text-sm font-bold text-gray-600 font-cairo">{{ $violation->type }}</span>
                                    <p class="text-[10px] text-gray-400 mt-1 line-clamp-1 max-w-xs">{{ $violation->description }}</p>
                                </td>
                                <td class="px-8 py-6 text-center">
                                    ...
                                    @endphp
                                    <span class="px-3 py-1.5 rounded-lg text-xs font-black {{ $severityClasses[$violation->severity] ?? 'bg-gray-100 text-gray-600' }}">
                                ...
                                <td class="px-8 py-6 text-center">
                                    <span class="text-xs font-bold text-navy font-cairo">{{ $violation->recordedBy->name ?? '---' }}</span>
                                </td>
                                <td class="px-8 py-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('violations.destroy', $violation->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه المخالفة؟')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-9 h-9 bg-red-50 text-red-500 rounded-lg flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
