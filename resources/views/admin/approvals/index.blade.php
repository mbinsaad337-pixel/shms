...
@section('content')
    <div class="mb-8 flex justify-between items-center bg-white p-6 rounded-2xl border-l-8 border-gold shadow-sm">
        ...
        <div class="flex gap-4">
            <span
                class="px-4 py-2 bg-navy/5 text-navy rounded-xl text-xs font-black font-cairo border border-navy/10 flex items-center gap-2">
        ...
        <div class="card-premium p-8 border-t-8 border-t-navy">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    <thead>
                        <tr
                            class="text-right text-gray-400 uppercase text-[10px] font-black tracking-widest font-cairo bg-gray-50">
                    ...
                    <tbody>
                        @if (is_countable($pendingBudgets)
                                ? count($pendingBudgets) > 0
                                : (method_exists($pendingBudgets, 'count')
                                    ? $pendingBudgets->count() > 0
                                    : !empty($pendingBudgets)))
                                ...
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="py-4 flex items-center gap-2">
                                        <a href="{{ route('budgets.show', $budget) }}"
                                            class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm font-cairo shadow-sm transition-all flex items-center gap-1">
                                            <i class="fas fa-eye"></i> عرض
                                        </a>
                                        <form action="{{ route('executive.budgets.approve', $budget) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            <button
                        ...
                        @else
                            <tr>
                                <td colspan="5" class="py-8 text-center text-gray-400 font-almarai">لا توجد طلبات اعتماد
                                    عهد
                                    حالياً</td>
                            </tr>
                        @endif
        ...
        <div class="card-premium p-8 border-t-8 border-t-gold">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    ...
                    <tbody>
                        @if (is_countable($pendingSettlements)
                                ? count($pendingSettlements) > 0
                                : (method_exists($pendingSettlements, 'count')
                                    ? $pendingSettlements->count() > 0
                                    : !empty($pendingSettlements)))
                                ...
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <form action="{{ route('executive.settlements.approve', $settlement) }}"
