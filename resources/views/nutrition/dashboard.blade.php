...
@section('title', 'لوحة متابعة التغذية')

@section('content')
    <div class="p-6 max-w-7xl mx-auto">

        @include('partials.print_header', (array)['title' => 'لوحة متابعة قسم التغذية', 'number' => 'FOOD-' . date('Ymd')])
            ...
            @foreach($cards as $card)
                <div
                    ...
                    <div class="relative z-10">
                        <div class="w-12 h-12 {{ $card['bg'] }} rounded-2xl flex items-center justify-center shadow-lg mb-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    @if(is_countable($recentBudgets) ? count($recentBudgets) > 0 : (method_exists($recentBudgets, 'count') ? $recentBudgets->count() > 0 : !empty($recentBudgets)))
    @foreach($recentBudgets as $budget)
                                @php /** @var \App\Models\FoodMonthlyBudget $budget */ @endphp
...
@else
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    @if(is_countable($recentInvoices) ? count($recentInvoices) > 0 : (method_exists($recentInvoices, 'count') ? $recentInvoices->count() > 0 : !empty($recentInvoices)))
    @foreach($recentInvoices as $invoice)
                        @php /** @var \App\Models\FoodPurchaseInvoice $invoice */ @endphp
...
@else
            ...
            <div class="card-premium p-6">
                ...
                <div class="space-y-3">
                    @if(is_countable($debtorStudents) ? count($debtorStudents) > 0 : (method_exists($debtorStudents, 'count') ? $debtorStudents->count() > 0 : !empty($debtorStudents)))
    @foreach($debtorStudents as $sub)
                        @php /** @var \App\Models\FoodMealSubscription $sub */ @endphp
                        <div
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4 no-print">
            ...
            @foreach($actions as $action)
                <a href="{{ route($action['route']) }}"
                    ...
                    <div
                        class="w-14 h-14 {{ $action['bg'] }} rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover:rotate-6 transition-all">
                        <i class="fas {{ $action['icon'] }} {{ $action['iconColor'] }} text-xl"></i>
                    </div>
                    <span class="font-black text-navy font-cairo text-xs text-center">{{ $action['label'] }}</span>
                </a>
            @endforeach
        </div>
    </div>
