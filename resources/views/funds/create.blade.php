            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                <div class="bg-primary px-10 py-8 text-white relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold font-cairo">صندوق جديد</h2>
                            <p class="text-white/80 font-almarai mt-1">إضافة صندوق مالي جديد للمركز</p>
                        </div>
                    </div>
                </div>

                <form action="{{ route('funds.store') }}" method="POST" class="p-10 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="مثال: الصندوق الرئيسي" value="{{ old('name') }}"
                            class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary outline-none text-right transition-all font-almarai">
                    ...
                    <div>
                        ...
                        <textarea name="description" placeholder="حدد الغرض من الصندوق..." rows="3"
                            class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary outline-none text-right transition-all font-almarai">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="balance" step="0.01" required placeholder="0.00" value="{{ old('balance') }}"
                            ...
                            <span id="currency_suffix" class="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 font-almarai text-xs">{{ currency_symbol(old('currency', 'YER')) }}</span>
                        </div>
                        @error('balance') <p class="text-red-500 text-xs mt-2 font-almarai">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex gap-4 pt-6">
                        <button type="submit" class="flex-[2] bg-primary text-white py-4 rounded-2xl font-bold text-lg shadow-lg font-cairo hover:bg-[#083358] transition-all flex items-center justify-center gap-2">
